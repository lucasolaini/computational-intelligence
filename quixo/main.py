...
class MinmaxPlayer(Player):
    ...
    def __init__(self, max_depth: int, player_idx: int, strategy: str, stopping_criteria: str = 'full', player_coeffs: list = [1, 20, 100, 1000, 10000],\
                opponent_coeffs: list = [1, 20, 100, 1000, 10000], verbose = False) -> None:
        super().__init__()
    ...
    def make_move(self, game: Game) -> tuple[tuple[int, int], Move]:
        ...
        if self.verbose:
            print(f"MINMAX TURN (PLAYER {self.player_idx})")
            self.print_board(game)

        moves = [((i, j), slide) for (i, j), slide in self.valid_moves if game._board[j, i] == self.player_idx or game._board[j, i] < 0]
        # evaluate each valid move
        best_move = deepcopy(moves[0])
        best_value = -np.inf if self.strategy == 1 else np.inf

        for move in moves:
            game_copy = deepcopy(game)
            game_copy._Game__move(move[0], move[1], self.player_idx)
            value = self.minmax(game_copy, self.max_depth, 1-self.strategy, 1-self.player_idx, best_value)
            # print("move:", move, "val:", value)
    ...
    def minmax(self, game: Game, depth: int, strategy: int, current_player_idx: int, last_value: int) -> int:
        ...
        if self.stopping_criteria == 'full':
            termination = 1
        elif self.stopping_criteria == 'depth':
            termination = (1 / (self.max_depth - depth + 1))

        if depth == 0 or game.check_winner() != -1 or random.random() > termination:
        ...
        best_value = -np.inf if strategy == 1 else np.inf
        ...
        for move in moves:
                ...
                value = self.minmax(game_copy, depth-1, 1-strategy, 1-current_player_idx, best_value)
                ...
                if (strategy == 0 and best_value <= last_value) or (strategy == 1 and best_value >= last_value):
                    return best_value
...
def are_symmetric(a: np.array, b: np.array):
    for i in range(4):
        if np.array_equal(a, b) or np.array_equal(np.flip(a, 0), b) or np.array_equal(np.flip(a, 1), b):
            return True
        a = np.rot90(a)
    return False
